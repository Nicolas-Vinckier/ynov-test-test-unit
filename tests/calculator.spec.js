...
test.describe('Cand\'culatrice E2E Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Aller sur la page d'accueil de la calculatrice
    await page.goto('/');
  });

  test('should display the initial calculator layout', async ({ page }) => {
  ...
  test('should handle simple local number input and clear', async ({ page }) => {
    ...
    await expect(page.locator('#current-operand')).toHaveText('');
    await expect(page.locator('#previous-operand')).toHaveText('');
    await expect(page.locator('#status-bar')).toContainText('Prêt');
  });

  test('should request calculation and show result (mocked API)', async ({ page }) => {
    ...
    await expect(page.locator('#status-bar')).toContainText('Calculé avec succès');
  });

  test('should handle cached API results', async ({ page }) => {
    ...
    await expect(page.locator('#status-bar')).toContainText('Résultat en cache');
    await expect(page.locator('.dot')).toHaveClass(/cached/);
  });

  test('should display error when API returns a calculation error', async ({ page }) => {
    ...
    await expect(page.locator('#status-bar')).toContainText('Erreur: Division par zéro impossible');
